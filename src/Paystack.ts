 ...
 * @returns 
 */
export function selectIntegration (integrations: IIntegration[], token: string) {
    const [command] = useCommand()

    return new Promise<IIntegration>(async (resolve, reject) => {
 ...
 * @returns 
 */
export function pingWebhook (options: XGeneric, event: XEvent = 'charge.success') {
    const [command] = useCommand()

    return new Promise<{ code: number, text: string, data: XGeneric | string }>(async (resolve, reject) => {
        let canProceed;
        try {
